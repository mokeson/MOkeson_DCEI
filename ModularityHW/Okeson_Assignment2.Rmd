...
<!---chunk for removing na values and locating stations--->
```{r echo=F, cache=T}

temp.na<-na.omit(dt.temp)
pcpn.na<-na.omit(dt.pcpn)

temp.loc<-list(temp.na$lat,temp.na$lon)
pcpn.loc<-list(pcpn.na$lat,pcpn.na$lon)
...
gt40<-aggregate(dt.temp, by= list(temp.loc),FUN=mean)
